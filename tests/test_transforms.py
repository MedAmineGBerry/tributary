...
class TestGeometricAdstock:
    ...
    @given(
        arrays(np.float64, shape=st.integers(4, 52), elements=st.floats(0, 1000, allow_nan=False)),
        st.floats(0.01, 0.99),
    )
...
class TestTransformPipeline:
    ...
    def test_adstock_then_saturation(self, normalized_spend_series, adstock_params, saturation_params):
    ...
    def test_all_channels_transform(self, normalized_spend_series, channel_transform_params):
        ...
        for channel, params in channel_transform_params.items():
            ...
            assert np.all(np.isfinite(saturated)), f"{channel} produced non-finite values"
